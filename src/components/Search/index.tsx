...
const SearchPopup: React.FC<SearchProps> = ({ fieldsToMap, updateOnFileChange }) => {
  ...
  useEffect(() => {
    if (predictionState && predictionState.currentPredictionEqualsString === true) updateOnFileChange(input);
    else updateOnFileChange(null);
  ...
  function applySuggest() {
    if (input.length === 0 && folderPrediction !== null && desiredPrediction !== null) {
      const prediction = makeInputPrediction(folderPrediction[desiredPrediction], fieldsToMap);
      ...
      setInput(resultPath);
      setPredictionState(makeInputPrediction(resultPath, fieldsToMap));
    } else {
      ...
      if (
        ...
        const newPrediction = makeInputPrediction(newPath, fieldsToMap);
  ...
  return (
    <div className='search-popup'>
      ...
      />
      <SearchButton predictionState={predictionState} input={input} fieldsToMap={fieldsToMap} />
    </div>
  );
};
